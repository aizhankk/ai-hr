...
class CandidateService:
    async def get_by_user_id(self, user_id: str) -> dict:
        async with database.db_pool.acquire() as conn:
            row = await conn.fetchrow(
                "SELECT * FROM aihr.candidate_profiles WHERE user_id = $1::uuid",
                user_id,
            )
            if not row:
    ...
    async def update(self, user_id: str, data: dict) -> dict:
        ...
        query = (
            f"UPDATE aihr.candidate_profiles "
    ...
    async def add_skill(
        ...
        async with database.db_pool.acquire() as conn:
            row = await conn.fetchrow(
                """
                INSERT INTO aihr.candidate_skills (candidate_id, skill_name, level, years_experience)
    ...
    async def remove_skill(self, candidate_id: str, skill_id: str) -> None:
        async with database.db_pool.acquire() as conn:
            result = await conn.execute(
                "DELETE FROM aihr.candidate_skills WHERE id = $1::uuid AND candidate_id = $2::uuid",
                skill_id,
                candidate_id,
            )
